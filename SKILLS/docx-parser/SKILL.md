...
| section 值 | 含义 | 检测规则 |
|---|---|---|
| `"cover"` | 封面页 | 文档开头到第一个 Heading 1 之间的段落 |
| `"toc"` | 目录页 | 包含"目录"/"目 录"/"目  录"/"Table of Contents"/"Contents"文本的段落，或样式名含"TOC"的段落 |
| `"body"` | 正文 | 其余所有段落 |

识别逻辑：
1. 遍历段落，遇到第一个 Heading 1 时标记封面页结束
2. 在封面页区域内检测目录关键词，标记目录页起止
3. 如果目录页结束位置未确定，向后查找直到遇到 Heading 1 或长文本段落
4. 如果未找到 Heading 1，使用备选策略查找第一个有实质内容的 Heading
