...
- 必须：使用 Word 内置标题样式（Heading 1/2/3），确保 TOC 正确识别
- 必须：标题颜色强制设置为黑色（RGB 0,0,0），避免继承蓝色主题色
...
### 上下文感知规则
- 短段落（<=40字）且前后都是长段落（>100字）→ 判定为二级标题
- 短段落前面是空段落且后面是长段落（>80字）→ 判定为二级标题
- 短段落（<=25字）前面是短段落且后面是长段落（>120字）→ 判定为二级标题
